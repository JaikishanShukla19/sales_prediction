...
def test_check():
    # all good
    df = pd.DataFrame(
        columns=["date", "id1", "id2", "val"], data=[[datetime.now(), "A", "B", 1.0]]
    )
    mdi = ForecastingMetadataInput(
        timestamp_column="date", id_columns=["id1", "id2"], target_columns=["val"]
    )
    ...
    df = pd.DataFrame(
        columns=["date", "id1", "id2", "val"], data=[[datetime.now(), "A", 1.0, 1.0]]
    )
    rc, ex = check(data=df, schema=mdi.model_dump())
    assert rc == 1
    ...
    mdi = ForecastingMetadataInput(
        timestamp_column="timestamp", id_columns=["id1", "id2"], target_columns=["val"]
    )
    rc, ex = check(data=df, schema=mdi.model_dump())
    ...
    mdi = ForecastingMetadataInput(
        timestamp_column="date", id_columns=["id1", "foobar"], target_columns=["val"]
    )
    rc, ex = check(data=df, schema=mdi.model_dump())
